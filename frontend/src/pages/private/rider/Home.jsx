...
export default function Home() {
    ...
    const recentCompletedOrders = completedOrders.filter(order => 
        new Date(order.updatedAt) >= last7Days
    );
    
    // Fixed delivery fee of ₱15 per delivery
    const deliveryFee = 15;
    const totalEarnings = recentCompletedOrders.length * deliveryFee;
    const averageOrderValue = recentCompletedOrders.length > 0 ? totalEarnings / recentCompletedOrders.length : 0;
        ...
        <RiderLayout>
            <div className="min-h-screen bg-[#232323] p-4">
                <div className="max-w-4xl mx-auto space-y-6">
                        ...
                        <CardContent className="space-y-3">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                >
                                    ...
                                    {availableLoading || availableFetching ? 'Refreshing...' : 'Refresh Orders'}
                                </Button>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[#FFC107] text-sm font-medium">Loading...</span>
                                </div>
                            </div>
                            <div className="space-y-4">
                                {[1, 2, 3].map((i) => (
                    ...
                    {/* Empty State */}
                    {!availableLoading && !availableFetching && availableOrders.length === 0 && activeOrders.length === 0 && (
                        <Card className="bg-[#2A2A2A] border-[#444]">
                            <CardContent className="p-8 text-center">
                                <Package size={60} className="mx-auto mb-4 text-gray-500" />
                                <h3 className="text-white text-lg font-semibold mb-2">No Orders Available</h3>
                                <p className="text-gray-400 text-sm mb-4">
                                    There are currently no orders waiting to be accepted or in progress.
                                </p>
                                <Button 
                                    variant="yellow" 
                                    onClick={() => refetchAvailableOrders()}
