...
export default function Cart() {
    ...
    function groupCartItems(items) {
        ...
        items.forEach(item => {
            ...
            const addOnsKey = Array.isArray(item.addOns) && item.addOns.length > 0
                ? item.addOns.map(a => a.addonId || a._id || a.name).sort().join(',')
                : '';
    ...
    function getGroupKey(item) {
        return item._id + (item.size || '') + (item.addOns && item.addOns.map(a => a.addonId || a._id || a.name).sort().join(','));
    }
    ...
    const selectedCart = groupedCart.filter(item => selectedKeys.includes(getGroupKey(item)));
    const subtotal = selectedCart.reduce((sum, item) => sum + (getItemUnitTotal(item) * item.quantity), 0);
    const discount = 0;
    ...
    return (
        <CustomerLayout>
            <div className="min-h-screen bg-[#232323] flex flex-col items-center py-10 px-2">
                <h1 className="text-4xl font-extrabold text-center text-white mb-8">Cart</h1>
                {/* Cart Items */}
                <div className="flex flex-wrap gap-6 justify-center mb-8 w-full max-w-5xl">
                    {/* Select All Checkbox */}
                    {groupedCart.length > 0 && (
                        <div className="w-full flex items-center mb-4 px-2 gap-2 relative z-10" style={{ minHeight: 32 }}>
                            <Checkbox
                                checked={allSelected}
                    ...
                    {loading ? (
                        ...
                        groupedCart.map(item => (
                            <div key={getGroupKey(item)} className="bg-white rounded-2xl shadow py-4 px-4 flex w-[320px] min-h-[140px] relative">
                                {/* Checkbox for selection */}
                                <Checkbox
                                    checked={selectedKeys.includes(getGroupKey(item))}
                                    onCheckedChange={() => toggleSelectOne(getGroupKey(item))}
                                ...
                                <div className="flex-1 flex flex-col justify-center min-w-0">
                                    <div className="font-bold text-lg text-[#232323] truncate">{item.productName}</div>
                                    {item.size && <div className="text-xs text-gray-500 font-semibold">{item.size}</div>}
                                    {item.addOns && item.addOns.length > 0 && (
                                        <div className="mt-1">
                                            <div className="text-xs text-[#FFC107] font-bold">Add-ons:</div>
                                            <ul className="text-xs text-[#232323]">
                                                {item.addOns.map((addon, idx) => (
                                                    <li key={addon.addonId || addon._id || idx} className="flex justify-between">
                                    ...
                                    <div className="text-[#232323] text-base font-bold mt-2">
                                        ...
                                        <span className="block text-xs font-normal text-gray-400 mt-1">
                                            ...
                                            {item.addOns && item.addOns.length > 0 &&
                                                <> + ₱ {item.addOns.reduce((sum, a) => sum + (Number(a.price) || 0), 0).toLocaleString()} add-ons</>
                                            }
                                            ) × {item.quantity}
                                        </span>
                                    </div>
                                ...
                                <div className="flex flex-col items-center justify-center gap-2 ml-4">
                                    {item.quantity > 1 ? (
                                        <Button variant="outline" size="icon" className="rounded-full w-8 h-8 flex items-center justify-center"
                                        ...
                                        >
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="20" height="20">
                                                <path fillRule="evenodd" d="M7.5 3a1 1 0 0 1 1-1h3a1 1 0 0 1 1 1V4h3.25a.75.75 0 0 1 0 1.5h-.278l-.427 9.399A2.25 2.25 0 0 1 13.1 17.13l-.1.007H7a2.25 2.25 0 0 1-2.25-2.224L4.323 5.5H4.25a.75.75 0 0 1 0-1.5H7.5V3zm1 1v1h3V4h-3zm-2.177 1.5l.427 9.399a.75.75 0 0 0 .75.726h6a.75.75 0 0 0 .75-.726l.427-9.399H6.323z" clipRule="evenodd" />
                                            </svg>
                                        </button>
                                    )}
                                    <span className="font-bold text-[#232323]">{item.quantity}</span>
                                    <Button variant="yellow" size="icon" className="rounded-full w-8 h-8 flex items-center justify-center"
                                        onClick={() => handleUpdateQuantity(item, 1)}
                                    >
                                        +
                                    </Button>
                ...
                <div className="w-full max-w-5xl flex justify-start mb-6">
                    <button className="text-white font-bold text-lg hover:underline">Apply promocode &gt;</button>
                </div>
                {/* Summary */}
