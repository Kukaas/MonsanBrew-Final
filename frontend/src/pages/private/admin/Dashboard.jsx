...
export default function Dashboard() {
    ...
    const { data: summaryData, isLoading: summaryLoading, refetch: refetchSummary } = useQuery({
        ...
        queryFn: async () => {
            const filters = {};
            if (dateRange?.from) filters.startDate = dayjs(dateRange.from).format('YYYY-MM-DD');
            if (dateRange?.to) filters.endDate = dayjs(dateRange.to).format('YYYY-MM-DD');
    ...
    const { data: salesData, isLoading: salesLoading } = useQuery({
        ...
        queryFn: async () => {
            if (groupBy === 'month') {
                ...
                const year = dateRange?.from ? dayjs(dateRange.from).year() : dayjs().year();
    ...
    if (groupBy === 'month') {
        ...
        formattedSalesData = (salesChartData || []).map(item => ({
            ...item,
            month: typeof item.month === 'number' ? dayjs().month(item.month - 1).format('MMMM') : item.month,
        }));
    ...
    const recentOrdersColumns = [
        ...
        },
        {
            ...
            header: 'Status',
            cell: ({ row }) => (
                <span className={`${getStatusColor(row.original.status)} border rounded-full px-3 py-1 text-xs font-medium`}>
            ...
            <div className="max-w-7xl mx-auto mt-8 p-6 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-[#BDBDBD] text-lg mt-1">
                            Welcome to your admin dashboard. Monitor your business performance and manage operations.
                        </p>
                    </div>
                    <Button
                        onClick={() => {
                ...
                <div className="grid grid-cols-1 gap-6">
                    ...
                    <DashboardAreaChart
                        data={
                          groupBy === 'month'
                            ? formattedSalesData.map(item => ({ ...item, date: item.month }))
                            : groupBy === 'day'
                              ? formattedSalesData.map(item => ({ ...item, date: item.day }))
                              : groupBy === 'week'
                                ? formattedSalesData
                                : formattedSalesData
                        }
                        ...
                        areaKeys={['sales']}
                        dateKey={groupBy === 'month' ? 'month' : groupBy === 'day' ? 'day' : groupBy === 'week' ? 'week' : 'date'}
                        defaultTimeRange={groupBy === 'month' ? '90d' : groupBy === 'week' ? '30d' : '7d'}
                        timeRangeOptions={[
                    ...
                    <Card className="bg-[#242424] border-[#292929]">
                        ...
                        <CardContent>
                            <DataTable
                                ...
                                data={recentOrders.map(order => ({ ...order, id: order._id }))}
                    ...
                    <Card className="bg-[#242424] border-[#292929]">
                        ...
                        <CardContent>
                            <DataTable
                                ...
                                rowProps={(row) => ({
                                    style: { cursor: 'pointer' },
                                    onClick: () => navigate(`/admin/raw-materials?highlight=${row.original._id}`)
                                })}
                            />
                        </CardContent>
                    </Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {topProducts.map((product, index) => (
                                    <div key={product._id} className="bg-[#2A2A2A] p-4 rounded-lg border border-[#333]">
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-white font-semibold truncate">{product.productName}</h4>
                                        ...
                                        <div className="space-y-1 text-sm text-[#BDBDBD]">
                                            ...
                                            <p>Rating: {product.averageRating?.toFixed(1) || '0.0'} ⭐</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
