...
export const createReview = async (req, res) => {
    try {
        const { userId, productId, orderId, rating, comment, isAnonymous } = req.body;
        ...
        if (!orderItem) {
            return res.status(400).json({ 
                ...
                debug: {
                    ...
                    availableProductIds: order.items.map(item => item.productId?.toString())
                }
            });
        }
...
const maskName = (name) => {
    ...
    // For names with 3+ characters, show first and last letter with asterisks in between
    if (name.length >= 3) {
        const maskedMiddle = '*'.repeat(name.length - 2);
        return firstChar + maskedMiddle + lastChar;
    }
    
    // For very short names, just show first letter with asterisks
    return firstChar + '***';
};
