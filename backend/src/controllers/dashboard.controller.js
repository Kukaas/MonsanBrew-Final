...
export const getDashboardSummary = async (req, res) => {
    try {
        ...
        // Get total sales (only completed orders)
        const completedOrdersFilter = { ...orderFilter, status: 'completed' };
...
export const getSalesData = async (req, res) => {
    try {
        ...
        switch (groupBy) {
            case 'day':
                dateFormat = '%Y-%m-%d';
                groupByField = { $dateToString: { format: dateFormat, date: '$createdAt' } };
                break;
            case 'week':
                dateFormat = '%Y-W%U';
                groupByField = { $dateToString: { format: dateFormat, date: '$createdAt' } };
...
export const getSalesDataWeekly = async (req, res) => {
    try {
        ...
        let weekSet = new Set(salesData.map(item => `${item._id.year}-W${item._id.week}`));
        while (current <= end) {
            const year = current.getFullYear();
            const week = getISOWeek(current);
            const key = `${year}-W${week}`;
            const existing = salesData.find(item => item._id.year === year && item._id.week === week);
            filledData.push({
                week: key,
